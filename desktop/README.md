...
- The bundled `market-pulse-api.exe` serves the React build and all `/api/*` routes on `127.0.0.1:8765`, and runs the background market/model scheduler in the same process.
...
To enable background model coordination in the scheduler, set `LLM_AUTO_ANALYSIS=true` (the Settings page writes this value for you). It runs only during trading sessions and defaults to one cross-market analysis every three minutes.
