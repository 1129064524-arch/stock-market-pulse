...
export default function Sidebar({ activeNav, menuOpen, onNavigate, onCloseMenu }) {
  return <>
    <aside className={`sidebar ${menuOpen ? 'open' : ''}`}>
      <div className="brand"><span className="brand-mark"><Activity size={19} /></span><span>市场脉冲</span></div>
      ...
      <div className="sidebar-footer"><button className={`nav-item ${activeNav === '设置' ? 'active' : ''}`} onClick={() => onNavigate('设置')}><Settings2 size={18} /><span>偏好设置</span></button><p>分析仅供研究参考<br />不构成投资建议</p></div>
    </aside>
