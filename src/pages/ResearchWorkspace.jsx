  ...
  return <section className="workspace-page research-workspace">
    ...
    <div className="research-layout">
      <article className="panel market-research">
        ...
        {analysisState === 'success' && analysis ? <div className="analysis-result research-result"><div className={`stance ${analysis.stance}`}>{analysis.stance}</div><div className="analysis-summary"><strong>{analysis.summary}</strong><EvidenceMeta coverage={analysis.evidence_coverage} refs={analysis.evidence_refs} /><span>{analysis.disclaimer}</span></div><div className="analysis-column"><h3>观察依据</h3><ul>{analysis.evidence.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="analysis-column risk-column"><h3>已知风险</h3><ul>{analysis.risks.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="analysis-column watch-column"><h3>继续跟踪</h3><ul>{analysis.watchlist.map((item) => <li key={item.code}><strong>{item.name}</strong><span>{item.reason}</span></li>)}</ul></div></div> : <div className={`analysis-empty ${analysisState === 'error' ? 'error' : ''}`}><BrainCircuit size={22} /><div><strong>{analysisState === 'error' ? '无法生成研判' : '等待生成市场研判'}</strong><p>{analysisState === 'error' ? analysisError : llmConfigured ? '生成后会把当前快照拆成证据、风险和继续跟踪项。' : '在 .env 中配置兼容模型地址、密钥和模型名称。'}</p></div></div>}
      </article>
