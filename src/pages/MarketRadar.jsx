...
export default function MarketRadar({
  ...
  return <>
    <section className="market-strip" aria-label="市场指数">
      ...
      <div className="market-card compact"><span>数据来源</span><strong>{liveOverview?.source === 'eastmoney' ? '东方财富' : liveOverview?.source === 'cache' ? '本地缓存' : '演示数据'}</strong><span className="neutral-label">{liveOverview?.as_of ? new Date(liveOverview.as_of).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }) : '等待连接'}</span></div>
    </section>
    <section className="radar-grid">
      <article className="panel movers-panel">
        ...
        <footer className="panel-footer"><span><span className={apiAvailable && liveOverview?.source === 'eastmoney' ? 'live-dot' : 'offline-dot'} />{apiAvailable ? marketSourceLabel : '演示数据模式'}</span><button onClick={() => setActiveNav('信号池')}>查看 {liveMovers.length} 个高异动信号 <ArrowUpRight size={15} /></button></footer>
      </article>
